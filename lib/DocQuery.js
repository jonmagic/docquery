...
var DocQuery = (function () {
  function DocQuery(directoryPath, options) {
    ...
    var isExtensionValid = function isExtensionValid(filePath) {
      return _this.extensions.find(function (x) {
        return x == path.extname(filePath);
      }) ? true : false;
    };
    ...
    this.watcher.on("add", function (filePath) {
      if (isExtensionValid(filePath)) _this.addDocument(fileDetails(filePath));
    });
    this.watcher.on("change", function (filePath) {
      if (isExtensionValid(filePath)) _this.updateDocument(fileDetails(filePath));
    });
    this.watcher.on("unlink", function (filePath) {
      if (isExtensionValid(filePath)) _this.removeDocument(_this._documents[filePath]);
    });
